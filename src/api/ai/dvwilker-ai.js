    ...
    app.get('/ai/dvwilker', async (req, res) => {
        ...
        if (!question) {
            return res.status(400).json({
                ...
                usage: {
                    example: '/ai/dvwilker?text=Hola, ¿cómo estás?',
                    params: {
        ...
        try {
            ...
            if (req.query.continue === 'true') {
                responseData.next_url = `/ai/dvwilker?text=${encodeURIComponent(question)}&id=${result.id}`;
            }
    ...
    app.get('/ai/dvwilker/continue', async (req, res) => {
        ...
        try {
            ...
            return res.status(200).json({
                ...
                response: result.text,
                conversation_id: result.id,
                next_url: `/ai/dvwilker/continue?text=${encodeURIComponent(text)}&id=${result.id}`
            });
